...
File::Locate - locate files within cached search paths (e.g. include files)

=head1 SYNOPSIS

    use File::Locate;

    $includes = File::Locate->new(Path => ['/usr/local/include',
					   '/usr/include']);
...
C<File::Locate> is useful for repeated file searches within a path of
directories.  It caches the contents of its search and supports two
different methods of fuzzy search, a normalize function and regular
expressions.  See the different METHODS for details.
...
=item B<new> - create a new File::Locate object

    $obj = File::Locate->new(Path =>
...
Example:
    ...
    $includes2 =
	File::Locate->new(Path => ['/usr/include!',
				   '/usr/include/sys!']);
